...
ships the internal API only; `GET /sync?since=` is part of that public contract and goes with it, to
M6+ and the client that needs it. The discipline that makes the OCS layer cheap later is kept:
controllers stay thin, all logic lives in services, no business rules in the route layer.
